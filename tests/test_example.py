...
def _verify_page_loaded(page: Page, timeout=10000):
    """Verify page has fully loaded and is interactive."""
    try:
...
def _verify_no_errors(page: Page):
    ...
    errors = page.evaluate("""
        () => {
            const errors = window.__playwright_errors__ || [];
            return errors;
        }
    """)
...
def test_example(page: Page):
    """
    Test navigation to Playwright docs with robust error handling and verifications.
    """
    ...
    try:
        ...
        page.screenshot(path="test_step_homepage.png")
        print("📸 Homepage screenshot saved")
        
        # Find "Get started" link with multiple strategies
        ...
        # If not found in main page, search iframes
        if not get_started:
        ...
        print("✅ 'Get started' link found")
        
        # Verify link is clickable
        expect(get_started).to_be_visible()
        expect(get_started).to_be_enabled()
        
        # Highlight element before clicking (for debugging)
        get_started.highlight()
        page.wait_for_timeout(500)  # Brief pause to see highlight
        ...
        assert "/docs/intro" in current_url, f"Expected URL to contain '/docs/intro', got: {current_url}"
        
        # Verify page loaded
        _verify_page_loaded(page)
        
        # Take screenshot after navigation
        page.screenshot(path="test_step_docs_page.png")
        ...
        has_expected_keyword = any(keyword.lower() in heading_text.lower() for keyword in expected_keywords)
        assert has_expected_keyword, f"Heading '{heading_text}' doesn't contain expected keywords"
...
# Example usage with Playwright test runner
def test_playwright_example(page: Page):
    """Wrapper function for Playwright test runner."""
    test_example(page)